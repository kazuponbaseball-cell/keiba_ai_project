...
def build_summary(history: pd.DataFrame, raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for horse_name in EXPECTED_RUNNERS:
        ...
        combo = group[fast_mask & slow3_mask].copy()
        fast_only = group[fast_mask].copy()
        slow_only = group[slow3_mask].copy()
        non_combo = group[~(fast_mask & slow3_mask)].copy()
        ...
        fast_rate = int(fast_only["top3"].sum()) / len(fast_only) if len(fast_only) else math.nan
        slow_rate = int(slow_only["top3"].sum()) / len(slow_only) if len(slow_only) else math.nan
        ...
        rows.append(
            {
                ...
                "複合条件戦数": starts,
                "複合条件馬券内数": top3,
                "複合条件馬券内率": round(rate, 4) if pd.notna(rate) else pd.NA,
                "非複合条件馬券内率": round(non_rate, 4) if pd.notna(non_rate) else pd.NA,
                "高速持続5F馬券内率": round(fast_rate, 4) if pd.notna(fast_rate) else pd.NA,
                "上がり35秒以上馬券内率": round(slow_rate, 4) if pd.notna(slow_rate) else pd.NA,
                "複合条件平均着順": round(avg_finish, 2) if pd.notna(avg_finish) else pd.NA,
                "複合条件平均人気": round(avg_pop, 2) if pd.notna(avg_pop) else pd.NA,
                "複合条件人気着順差": round(pop_gain, 2) if pd.notna(pop_gain) else pd.NA,
                "重賞複合条件好走数": len(graded_combo_good),
                "重賞複合条件重み": round(graded_score, 2),
                "息入りにくい複合条件重み": round(no_breather_score, 2),
...
def main() -> None:
    ...
    detail_path = out_dir / "takarazuka_kinen_2026_fast5f_slow3f_fit_summary.csv"
    history_path = out_dir / "takarazuka_kinen_2026_fast5f_slow3f_fit_histories.csv"
    report_path = out_dir / "takarazuka_kinen_2026_fast5f_slow3f_fit_report.md"
    ...
    lines = [
        "# 宝塚記念2026 想定馬 高速持続5Fかつ上がり35秒以上適性",
        "",
        "複合条件: 芝1800m以上で、後半5Fが60.0秒以内、後半5F内に11秒台が4つ以上、かつレース上がり3Fが35.0秒以上。",
        "狙い: 後半から速いラップを長く踏みつつ、最後の3Fは35秒以上かかるタフな持続戦への対応を見る。",
        "",
        "## ランキング",
        "",
        "|評価|馬名|スコア|複合条件成績|重賞複合重み|息入り複合重み|平均着順|人気着順差|根拠|",
        "|---|---|---:|---|---:|---:|---:|---:|---|",
    ]
    for _, row in ranked.iterrows():
        avg_finish = f"{float(row['複合条件平均着順']):.2f}" if pd.notna(row["複合条件平均着順"]) else ""
        pop_gain = f"{float(row['複合条件人気着順差']):.2f}" if pd.notna(row["複合条件人気着順差"]) else ""
        lines.append(
            f"|{row['評価']}|{row['馬名']}|{float(row['適性スコア']):.2f}|"
            f"{record(int(row['複合条件戦数']), int(row['複合条件馬券内数']))}|"
            f"{float(row['重賞複合条件重み']):.1f}|{float(row['息入りにくい複合条件重み']):.1f}|"
    ...
    lines.extend(
        [
            ...
            "## 読み取り",
            "",
            "- 高速持続5Fだけで浮上した馬のうち、上がり35秒以上も同時に満たす実績がある馬を上位に残す。",
            "- 単なる切れ味型より、速いロンスパを踏んで最後も踏ん張る馬を評価する条件。",
