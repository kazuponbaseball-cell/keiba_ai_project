...
def tendency(value: object) -> str:
    diff = pd.to_numeric(pd.Series([value]), errors="coerce").iloc[0]
    if pd.isna(diff):
        return "不明"
    if diff <= -0.6:
        return "前傾"
...
def record_text(frame: pd.DataFrame) -> str:
    ...
    win = int(finish.eq(1).sum())
    second = int(finish.eq(2).sum())
    third = int(finish.eq(3).sum())
    out = int((finish.ge(4) & finish.le(99)).sum())
    return f"{win}-{second}-{third}-{out}"
...
def best_examples(frame: pd.DataFrame, n: int = 2) -> str:
    if frame.empty:
        return ""
    rows = frame.copy()
    ...
    for _, row in rows.iterrows():
        ...
        parts.append(
            ...
            f"{int(row['finish'])}着 {pci} {rpci}".strip()
        )
    return " / ".join(parts)
...
def build_summary(history: pd.DataFrame, raw: pd.DataFrame) -> pd.DataFrame:
    ...
    for horse_name in EXPECTED_RUNNERS:
        ...
        front = group[group["ペース傾向"].eq("前傾")]
        middle = group[group["ペース傾向"].eq("ミドル")]
        back = group[group["ペース傾向"].eq("後傾")]
        rows.append(
            {
                "馬名": horse_name,
                "判定対象": len(group),
                "前傾成績": record_text(front),
                "前傾平均着順": avg_finish(front),
                "ミドル成績": record_text(middle),
                "ミドル平均着順": avg_finish(middle),
                "後傾成績": record_text(back),
                "後傾平均着順": avg_finish(back),
                "前傾好走例": best_examples(front[front["finish"].between(1, 3)]),
                "後傾好走例": best_examples(back[back["finish"].between(1, 3)]),
            }
        )
...
def main() -> None:
    ...
    history["ペース傾向"] = history["front_minus_last"].apply(tendency)
    summary = build_summary(history, raw)

    detail_path = out_dir / "takarazuka_kinen_2026_front_back_records_summary.csv"
    history_path = out_dir / "takarazuka_kinen_2026_front_back_records_histories.csv"
    report_path = out_dir / "takarazuka_kinen_2026_front_back_records_report.md"
    ...
    lines = [
        "# 宝塚記念2026 想定馬 前傾/後傾成績",
        "",
        "定義: 前半3F - 後半3F が -0.6秒以下なら前傾、+0.6秒以上なら後傾、その間をミドル。",
        "成績表記は `(1着-2着-3着-圏外)`。",
        "",
        "|馬名|対象|前傾|前傾平均|ミドル|ミドル平均|後傾|後傾平均|",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ]
    for _, row in summary.iterrows():
        lines.append(
            f"|{row['馬名']}|{int(row['判定対象'])}|{row['前傾成績']}|{row['前傾平均着順']}|"
            f"{row['ミドル成績']}|{row['ミドル平均着順']}|{row['後傾成績']}|{row['後傾平均着順']}|"
        )
    lines.extend(
        [
            "",
    ...
    for _, row in summary.iterrows():
        lines.append(f"|{row['馬名']}|{row['前傾好走例']}|{row['後傾好走例']}|")
    lines.extend(
        [
            "",
            "## 出力ファイル",
