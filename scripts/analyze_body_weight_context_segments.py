...
def add_body_context(df: pd.DataFrame, race_col: str) -> pd.DataFrame:
    out = df.copy()
    ...
    out["body_weight_z_in_race"] = (
        (weight - weight.groupby(out[race_col]).transform("mean"))
        / weight.groupby(out[race_col]).transform("std").replace(0, np.nan)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Analyze body-weight effects by distance, surface, and course size.")
    ...
    parser.add_argument("--output-dir", default="outputs/analysis/body_weight_context_segments")
    ...
    scored = add_body_context(scored, race_col)
    ...
    for col in ["surface_simple", "distance_bucket_simple", "course_size", "body_weight_bucket", "場所"]:
        for value in sorted(scored[col].dropna().unique()):
            masks[f"{col}={value}"] = scored[col].eq(value)

    rows: list[dict[str, object]] = []
    combos: list[tuple[str, ...]] = []
    for body in ["big_500", "big_520", "mid_460_499", "small_460", "race_heavy_top3", "race_heavy_top5"]:
        combos.append((body,))
        for ai in ["ai_top1", "ai_top3", "ai_top5"]:
            combos.append((ai, body))
            for surface in ["surface_simple=turf", "surface_simple=dirt"]:
                combos.append((ai, body, surface))
            for distance in [
                ...
                combos.append((ai, body, distance))
            for size in ["course_size=big_box", "course_size=local_tight", "course_size=middle_mixed"]:
                combos.append((ai, body, size))
            for surface, distance in combinations(
                [
                    "surface_simple=turf",
                    "surface_simple=dirt",
                    "distance_bucket_simple=sprint_1300",
                    ...
                    "distance_bucket_simple=middle_2000",
                    "course_size=big_box",
                    "course_size=local_tight",
                    "course_size=middle_mixed",
                ],
                2,
            ):
                if surface.split("=")[0] == distance.split("=")[0]:
                    continue
                combos.append((ai, body, surface, distance))
    ...
    for combo in combos:
        ...
        if len(discovery_part) < max(30, args.min_bets // 3) or len(validation_part) < max(30, args.min_bets // 3):
            continue
        label = "&".join(combo)
        rows.append(
            {
    ...
    bucket_rows = []
    group_cols = ["surface_simple", "distance_bucket_simple", "course_size", "body_weight_bucket"]
    for group_col in group_cols:
        for value, part in scored.groupby(group_col, sort=False):
            bucket_rows.append({"group": group_col, "value": value, **metrics(part, race_col, str(value))})
    bucket_df = pd.DataFrame(bucket_rows)

    output_dir = ensure_dir(project_path(args.output_dir))
    out.to_csv(output_dir / "body_weight_context_segments.csv", index=False, encoding="utf-8-sig")
    out.head(100).to_csv(output_dir / "top_body_weight_context_segments.csv", index=False, encoding="utf-8-sig")
    bucket_df.to_csv(output_dir / "body_weight_context_bucket_metrics.csv", index=False, encoding="utf-8-sig")
    summary = {
