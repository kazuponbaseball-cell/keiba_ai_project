...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check Victoria Mile results for Niigata turf 1600 and Sekiya Kinen history.")
    ...
    sekiya_top3_ids = set(
        df[df["レース名"].astype(str).isin(SEKIYA_NAMES) & (df["確定着順"] <= 3)]["血統登録番号"].dropna().astype(str)
    )

    vm = df[df["レース名"].astype(str) == "ヴィクトG1"].copy()
    vm["has_niigata1600_top3"] = vm["血統登録番号"].astype(str).isin(niigata1600_top3_ids)
    vm["has_sekiya_top3"] = vm["血統登録番号"].astype(str).isin(sekiya_top3_ids)
    vm["has_both"] = vm["has_niigata1600_top3"] & vm["has_sekiya_top3"]
    vm["has_niigata1600_only"] = vm["has_niigata1600_top3"] & ~vm["has_sekiya_top3"]
    vm["has_sekiya_only"] = ~vm["has_niigata1600_top3"] & vm["has_sekiya_top3"]
    vm["has_neither"] = ~vm["has_niigata1600_top3"] & ~vm["has_sekiya_top3"]

    summaries = [
        summarize(vm[vm["has_niigata1600_top3"]], "has_niigata1600_top3"),
        summarize(vm[vm["has_sekiya_top3"]], "has_sekiya_top3"),
        summarize(vm[vm["has_both"]], "niigata1600_and_sekiya_top3"),
        summarize(vm[vm["has_niigata1600_only"]], "niigata1600_top3_only"),
        summarize(vm[vm["has_sekiya_only"]], "sekiya_top3_only"),
    ...
    detail = (
        vm[
            [
                ...
                "人気",
                "確定着順",
                "has_niigata1600_top3",
                "has_sekiya_top3",
                "has_both",
            ]
        ]
        .sort_values(["日付", "確定着順", "人気"])
        .drop(columns=["日付"])
    )

    output_dir = ensure_dir(project_path(args.output_dir))
    run_dir = ensure_dir(output_dir / f"victoria_mile_niigata_sekiya_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
