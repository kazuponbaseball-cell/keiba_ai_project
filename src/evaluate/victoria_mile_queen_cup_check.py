...
def main() -> None:
    parser = argparse.ArgumentParser(description="Check Victoria Mile results for horses with past Queen Cup top-3 finishes.")
    ...
    queen_top3_ids = set(
        df[df["レース名"].astype(str).isin(QUEEN_CUP_NAMES) & (df["確定着順"] <= 3)]["血統登録番号"].dropna().astype(str)
    )

    vm = df[df["レース名"].astype(str) == "ヴィクトG1"].copy()
    vm["has_queen_cup_top3"] = vm["血統登録番号"].astype(str).isin(queen_top3_ids)

    summaries = [
        summarize(vm[vm["has_queen_cup_top3"]], "has_queen_cup_top3"),
        summarize(vm[~vm["has_queen_cup_top3"]], "no_queen_cup_top3"),
    ]

    detail = vm[vm["has_queen_cup_top3"]][["日付", "日付S", "馬名", "年齢", "人気", "確定着順"]].sort_values(
        ["日付", "確定着順", "人気"]
    ).drop(columns=["日付"])

    output_dir = ensure_dir(project_path(args.output_dir))
    run_dir = ensure_dir(output_dir / f"victoria_mile_queen_cup_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
