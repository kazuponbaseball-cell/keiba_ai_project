...
from src.pipelines.import_target_entry_csv import main as import_target_entry_main
...
def _latest_csv(directory: Path, patterns: list[str]) -> Path:
    ...
    if not files:
        raise FileNotFoundError(f"No TARGET entry CSV files found under {directory}")
    return max(files, key=lambda path: path.stat().st_mtime)


def main() -> None:
    parser = argparse.ArgumentParser(description="Import the newest TARGET entry CSV from the configured inbox.")
    ...
    entry_dir = ensure_dir(project_path(target_cfg["entry_drop_dir"]))
    latest = _latest_csv(entry_dir, list(target_cfg.get("entry_file_patterns", ["*.csv"])))

    summary = {
        "selected_input_csv": str(latest),
        "entry_drop_dir": str(entry_dir),
    }
    ...
    sys.argv = [
        ...
        "--alias-config",
        args.alias_config,
        *([] if args.feature_config is None else ["--feature-config", args.feature_config]),
